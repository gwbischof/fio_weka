...
with pushd( os.path.dirname( progname ) ):
    ...
    for script in fio_scripts:
        ...
        if reportitem["bandwidth"]:
            print "    total read bandwidth: " + format_units_bytes( readbytes ) + "/s"
            print "    total write bandwidth: " + format_units_bytes( writebytes ) + "/s"
            print "    total bandwidth: " + format_units_bytes( bw_bytes ) + "/s"
            print "    avg read bandwidth: " + format_units_bytes( float( readbytes )/float( len(jobs)-1 ) ) + "/s per host"
            print "    avg write bandwidth: " + format_units_bytes( float( writebytes )/float( len(jobs)-1 ) ) + "/s per host"
            print "    avg bandwidth: " + format_units_bytes( float( bw_bytes )/float( len(jobs)-1 ) ) + "/s per host"
        if reportitem["iops"]:
            print "    total read iops: " + ("{:,}".format(int(readiops))) + "/s"
            print "    total write iops: " + ("{:,}".format(int(writeiops))) + "/s"
            print "    total iops: " + ("{:,}".format(int(iops))) + "/s"
            print "    avg read iops: " + ("{:,}".format(int(readiops) / len(jobs)-1 )) + "/s per host"
            print "    avg write iops: " + ("{:,}".format(int(writeiops) / len(jobs)-1 )) + "/s per host"
            print "    avg iops: " + ("{:,}".format(int(iops) / len(jobs)-1 )) + "/s per host"
        if reportitem["latency"]:
            print "    read latency: " +  format_units_ns( float( readlat ) )
            print "    write latency: " +  format_units_ns( float( writelat ) )
